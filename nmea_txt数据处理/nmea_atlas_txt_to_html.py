...
for files in filelist:
 #Olddir = os.path.join(path1,files)
  ...
  #高斯正算,WGS84椭球,3度带
  if(utcgst==0):
	  print("%s"%filename+"	文件中没有GST数据,不能反映收敛趋势,请手动处理！！！")
	  continue
  if(utc4==0):
	  print("%s"%filename+"	文件中没有固定解数据,请手动处理!!!")
	  continue
  page=Page()
  rarr=EWZWC(x_datas4,y_datas4)
  es1 =Scatter("中国精度测试北坐标变化图",width=1200)
  ...
  page.add(es1)
  es2 =Scatter("中国精度测试东坐标变化图",width=1200)
  ...
  page.add(es2)
  es3 =Scatter("中国精度测试高程变化图",width=1200)
  ...
  page.add(es4)
  es5 =Scatter("中国精度测试Vrms变化图",width=1200)
